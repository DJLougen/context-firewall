...
class _NoOpLock:
    """No-op lock for non-thread-safe mode."""
    def __enter__(self):
        return self
...
class SessionState:
    ...
    def __init__(self, thread_safe: bool = True) -> None:
        ...
        # Thread safety: reentrant lock for all mutations
    ...
    def get_compression_ratio(self) -> float:
        ...
        with self._lock:
            original = sum(e.original_tokens for e in self.entries)
            compressed = self._cached_total_tokens
        if compressed == 0:
            return 0.0
